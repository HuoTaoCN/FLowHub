...
- **发送体验持续完善中**
  - 已接入原生文件选择对话框（`tauri-plugin-dialog`），可在 Send 页面通过“Browse…”按钮选取本地文件，无需手填路径
  - 已支持发送中任务的暂停/恢复、失败任务重试（带断点续传），后端通过 `transfer_id -> TransferControl` 注册表跟踪每个传输的控制句柄
  - 仍待完善：批量发送、发送历史持久化、跨会话恢复
- **设备发现仍是 MVP 级别**
